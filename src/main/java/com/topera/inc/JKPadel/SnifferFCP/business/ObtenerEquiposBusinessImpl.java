...
public class ObtenerEquiposBusinessImpl implements ObtenerEquiposBusiness{
	...
	@Override
	public Integer ObtenerEquipos(Integer anno) throws Exception {
...
//			List<Liga> ligas = ligaDAO.findAll();
	...
	public Integer ObtenerEquiposByLiga(Liga liga) throws Exception {
	...
	public Integer ObtenerEquiposByGrupo(Grupo grupo, Liga liga) throws Exception {
		...
		try {
			...
			for (Element htmlEquipo : ElinksEquipos) {
				...
				String linkAux = link;
				Integer idEquipo = Integer.parseInt(linkAux.substring(linkAux.indexOf("=") + 1, linkAux.indexOf("&")));
				linkAux = linkAux.substring(linkAux.indexOf("&") + 1);
				Integer idLiga   = Integer.parseInt(linkAux.substring(linkAux.indexOf("=") + 1, linkAux.indexOf("&")));
				linkAux = linkAux.substring(linkAux.indexOf("&") + 1);
				Integer idGrupo  = Integer.parseInt(linkAux.substring(linkAux.indexOf("=") + 1));
				...
				if (!equipoAux.isPresent()) {
					...
					equipo.setId_liga(idLiga);
					equipo.setId_grupo(idGrupo);
					equipo.setId_sede(idSede);
...
//				Integer idEquipoGrupo = obtenerEquipoGrupo(idEquipo, grupo.getId_grupo(), idSede);
...
//				Integer nJugadores = ObtenerJugadoresEquipo(link, grupo.getGenero(), liga, idEquipo);
	...
	public Integer ObtenerSedeByEquipo(String url) throws Exception {
...
//	private Integer obtenerEquipoGrupo(Integer idEquipo, Integer idGrupo, Integer idSede) {
//		
//		Integer retorno = -1;
//		
//		Optional<EquipoGrupo> GrupoEquipoAux = equipoGrupoDAO.findByIdEquipoGrupo(idEquipo, idGrupo);
//		
//		if (!GrupoEquipoAux.isPresent()) {
//			EquipoGrupo equipoGrupo = new EquipoGrupo();
//			equipoGrupo.setId_equipo(idEquipo);
//			equipoGrupo.setId_grupo(idGrupo);
//			equipoGrupo.setId_sede(idSede);
//			
//			EquipoGrupo EquipoGrupoInserted = equipoGrupoDAO.save(equipoGrupo);
//			
//			retorno = EquipoGrupoInserted.getId_sede();
//		}
//			
//		
//		return retorno;
